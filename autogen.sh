...
get_current_version() {
    ## XXXvlab: what to do if current workdir is unclean ?
...
[ -z "$FILES" ] && {
    FILES="setup.cfg setup.py CHANGELOG.rst pkg/*"
}
...
if [ -f configure.ac -o -f configure.in ]; then
    ...
    if ! get_path aclocal >/dev/null; then
    ...
    if ! get_path automake >/dev/null; then
    ...
    if ! get_path autoreconf >/dev/null; then
